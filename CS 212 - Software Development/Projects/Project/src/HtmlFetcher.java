...
public class HtmlFetcher {
    ...
    {
        // DONE Fill in isHtml(...) implementation.
        if (headers.containsKey("Content-Type"))
        {
            List<String> listOfStrings = headers.get("Content-Type");
            
			return (listOfStrings.get(0).startsWith("text/html"));
        }
    ...
    {
        // DONE Fill in getStatusCode(...) implementation.
        List<String> listOfStrings = headers.get(null);

		String regexExpression = "(?s)HTTP\\/\\d\\.\\d\\s(\\d*?)\\s.*?";
        
        Pattern pattern = Pattern.compile(regexExpression);
        Matcher matcher = pattern.matcher(listOfStrings.get(0));
    ...
    {
        // DONE Fill in isRedirect(...) implementation.
        return (getStatusCode(headers) >= 300 && getStatusCode(headers) < 400);
    }
    ...
    {
        // DONE Fill in fetch(...) implementation. Must use sockets, NOT a URLConnection object!
        URL redirectURL = null;

		Map<String, List<String>> mapOfURLs;
        
        try
        {
            mapOfURLs = HttpsFetcher.fetchURL(url);
            
            String line = String.join("\n", mapOfURLs.get("Content"));
            
            if (getStatusCode(mapOfURLs) >= 200 && getStatusCode(mapOfURLs) < 300)
            {
                if (isHtml(mapOfURLs))
                {
					return line;
            ...
            else
            {
                if (isRedirect(mapOfURLs))
                {
                    ...
                    {
                        if (mapOfURLs.containsKey("Location"))
                        {
                            List<String> listOfURLs = mapOfURLs.get("Location");
                            
                            try
                            {
								redirectURL = new URL(listOfURLs.get(0));
                            }
