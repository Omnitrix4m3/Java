...
public class SimpleReadWriteLock {
    ...
    private class WriteLock implements SimpleLock {
        ...
        // DONE: ADD MEMBERS AS NECESSARY
	    /** Initializes a thread writer to be used by the class */
        ...
        {
            /*
             * DONE: FILL IN THIS METHOD 1) Wait until safe to write 2) Update number of
             * active writers 3) Update which thread "holds" the write lock
             */
            synchronized (lock)
            {
        ...
        {
            /*
             * DONE: FILL IN THIS METHOD 1) Throw a ConcurrentModificationException it the
             * wrong thread is calling unlock() or if lock() has not yet been called.
             *
             * 2) Update the number of active writers
             *
             * 3) Update which thread "holds" the write lock (in this case, no thread should
             * hold the lock after unlock() finishes)
             */

            if (SimpleReadWriteLock.sameThread(threadWriter) && threadWriter != null)
            {
                synchronized (lock)
                {
					writers--;
